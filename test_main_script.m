...
close all;
rosshutdown
[Z_lat,X_lat,t_lat]=test_fn_lat();
[Z_long,X_long,t_long]=test_fn_long(); % changed 3rd step length to half step
X_h=zeros(size(X_lat));
% lh and rh assumes the ground plane is at z=-(distance from center of mass from calculation).
[ llat,llong,lh,rlat,rlong,rh ] = test_fn_foot_placement(); 
% %%
...
plot3(llat,llong,lh,rlat,rlong,rh,X_lat,X_long,X_h);
grid
%%
% clear all;
% close all;
...
rosinit('172.17.63.166','NodeHost','172.17.231.124');
...
% pr.Positions = [0 0 0 0 0 0];
% pl.Positions = [0 0 0 0 0 0];
...
pose.Pose.Position.X = -0.018;%0.58 0.377 0.46
